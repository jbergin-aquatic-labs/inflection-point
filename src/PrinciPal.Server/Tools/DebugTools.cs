...
/// Each method resolves the Result from the service, throwing <see cref="McpException"/> on failure.
/// </summary>
[McpServerToolType]
public class DebugTools
{
    ...
    public string ListSessions()
        => Resolve(_queryService.ListSessions());
    ...
    public string GetDebugState(
        ...
        int depth = 2)
        => Resolve(_queryService.GetDebugState(session, depth));
    ...
    public string GetLocals(
        ...
        [Description("Max member expansion depth (0=flat, 2=default)")]
        int depth = 2)
        => Resolve(_queryService.GetLocals(session, depth));
    ...
    public string GetCallStack(
        [Description("Session name or ID. Use list_sessions to see options.")]
        string session)
        => Resolve(_queryService.GetCallStack(session));
    ...
    public string GetSourceContext(
        [Description("Session name or ID. Use list_sessions to see options.")]
        string session)
        => Resolve(_queryService.GetSourceContext(session));
    ...
    public string GetBreakpoints(
        [Description("Session name or ID. Use list_sessions to see options.")]
        string session)
        => Resolve(_queryService.GetBreakpoints(session));
    ...
    public string GetExpressionResult(
        ...
        [Description("Max member expansion depth (0=flat, 2=default)")]
        int depth = 2)
        => Resolve(_queryService.GetExpressionResult(session, depth));
    ...
    public string ExplainCurrentState(
        [Description("Session name or ID. Use list_sessions to see options.")]
        string session)
        => Resolve(_queryService.ExplainCurrentState(session));
    ...
    public string GetBreakpointHistory(
        [Description("Session name or ID. Use list_sessions to see options.")]
        string session)
        => Resolve(_queryService.GetBreakpointHistory(session));
    ...
    public string GetSnapshot(
        ...
        [Description("Max member expansion depth (0=flat, 2=default)")]
        int depth = 2)
        => Resolve(_queryService.GetSnapshot(index, session, detail, depth));
    ...
    public string ExplainExecutionFlow(
        ...
        int count = 0)
        => Resolve(_queryService.ExplainExecutionFlow(session, detail, depth, start, count));

    private static string Resolve(Result<string> result) =>
        result.Match(
            onSuccess: value => value,
            onFailure: error => throw new McpException(error.Description));
}
